...
class PopArt_hatrpo(nn.Module):
    ...
    def __init__(self, input_shape, norm_axes=1, beta=0.99999, per_element_update=False, epsilon=1e-5, device=torch.device("cpu")):
        super(PopArt_hatrpo, self).__init__()
        ...
        # 使用设备兼容的方式创建tpdv
        from utils.device_utils import create_tpdv, tensor_to_device
        self.tpdv = create_tpdv(device)

        # 使用设备兼容的张量创建方式
        self.running_mean = nn.Parameter(tensor_to_device(torch.zeros(input_shape), device), requires_grad=False)
        self.running_mean_sq = nn.Parameter(tensor_to_device(torch.zeros(input_shape), device), requires_grad=False)
        self.debiasing_term = nn.Parameter(tensor_to_device(torch.tensor(0.0), device), requires_grad=False)
