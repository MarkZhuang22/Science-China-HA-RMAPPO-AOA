...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # 合并来自场景的全部信息键（例如 pz_breach_now_step 等）
        ...
        # 让场景有机会在每步之后做一些“后处理”（例如冻结被捕入侵机的速度）
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if isinstance(action_space, MultiDiscrete):
            ...
            action = [action]
        # 🔧 关键修复：检查智能体当前的movable状态，而不是动作空间配置
        # 这是因为post_step_callback可能在运行时改变智能体的movable状态
        if agent.movable:
            # physical action
            ...
            # 🔧 修复：正确处理action数组的消费逻辑
            if not isinstance(action_space, MultiDiscrete):
                if not agent.silent:
                    # 非静默智能体：从当前动作元素中移除已使用的物理维度
                    if len(action[0]) > d:
                        action[0] = action[0][d:]
                    # 如果action[0]恰好只有d维，保留空列表等待通信处理
                else:
                    # 静默智能体：没有通信动作，直接移除物理动作元素
                    action = action[1:]
        else:
            # 🔧 新增：处理不可移动智能体（比如被捕获的进攻机）
            # 跳过物理动作处理，但仍需消费动作数组以保持一致性
            if not isinstance(action_space, MultiDiscrete):
                if not agent.silent:
                    # 非静默智能体：从当前动作元素中移除物理维度
                    d = self.world.dim_p
                    if len(action[0]) > d:
                        action[0] = action[0][d:]
                    # 如果action[0]恰好只有d维，保留空列表等待通信处理
        ...
        if not agent.silent:
            ...
            if self.discrete_action_input:
                agent.action.c = np.zeros(self.world.dim_c)
                if len(action) > 0:  # 确保有剩余动作元素
                    agent.action.c[action[0]] = 1.0
            else:
                if len(action) > 0:  # 确保有剩余动作元素
                    agent.action.c = action[0]

            if len(action) > 0:  # 只在有剩余元素时才移除
                action = action[1:]
