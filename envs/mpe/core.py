...
class World(object):
    def __init__(self):
        ...
        self.dt = 0.1
        # 阻尼系数 - 降低阻尼减少边界碰撞后的减速
        self.damping = 0.15  # 从0.25降低到0.15
        # 碰撞参数 - 降低碰撞力减少边界反弹的影响
        self.contact_force = 50.0  # 从100降低到50
    ...
    def integrate_state(self, p_force):
        for i, entity in enumerate(self.entities):
            ...
            # ========== 强制边界约束（严格防止越界） ==========
            # 检查是否有世界边界半径设置
            if hasattr(self, '_pz_world_r'):
                world_r = self._pz_world_r
                # 强制约束智能体位置在边界内，不允许越界
                dist_from_center = np.sqrt(entity.state.p_pos[0]**2 + entity.state.p_pos[1]**2)
                max_allowed_dist = world_r - entity.size  # 严格限制在边界内
                if dist_from_center > max_allowed_dist:
                    # 强制拉回边界内
                    scale_factor = max_allowed_dist / dist_from_center
                    entity.state.p_pos *= scale_factor
                    # 清除向外的速度分量
                    vel_dot_pos = np.dot(entity.state.p_vel, entity.state.p_pos)
                    if vel_dot_pos > 0:  # 速度指向边界外
                        entity.state.p_vel -= vel_dot_pos / (dist_from_center**2) * entity.state.p_pos
    ...
    def get_entity_collision_force(self, ia, ib):
        ...
        # prevent division by zero when entities are at exactly the same position
        if dist < 1e-10:
            # use a small random offset to separate entities
            delta_pos = np.random.randn(2) * 1e-6
    ...
    def get_wall_collision_force(self, entity, wall):
        ...
        k = self.contact_margin
        penetration = np.logaddexp(0, -(dist - dist_min)/k)*k
        # prevent division by zero when entity is at exactly the wall position
        if dist < 1e-10:
            dist = 1e-10  # use small epsilon instead of zero
