...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == 'set_assignments':
            # Expect a dict mapping defender_idx -> intruder_idx (or -1 for idle)
            # Store on the underlying world for the scenario to consume.
            try:
                setattr(env.world, '_pz_assignments', data)
            except Exception:
                pass
            remote.send(None)
        else:
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def set_assignments(self, assignments_list):
        """Set per-env defender->intruder assignment dicts.
        assignments_list: list of dicts, length == num_envs.
        """
        assert len(assignments_list) == len(self.remotes)
        for remote, mapping in zip(self.remotes, assignments_list):
            remote.send(('set_assignments', mapping))
        # drain acks
        for remote in self.remotes:
            remote.recv()
...
class DummyVecEnv(ShareVecEnv):
    ...
    def set_assignments(self, assignments_list):
        """Set per-env defender->intruder assignment dicts for dummy envs."""
        assert len(assignments_list) == len(self.envs)
        for env, mapping in zip(self.envs, assignments_list):
            try:
                setattr(env.world, '_pz_assignments', mapping)
            except Exception:
                pass
